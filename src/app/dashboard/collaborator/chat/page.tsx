...
interface Thread { id: string; status: string; createdAt: string; messages: string }
...
export default function CollaboratorChatPage() {
  ...
  const load = () => fetch('/api/collaborator/chat').then(r => r.json()).then(d => { setThreads(d); setLoading(false); });
  useEffect(() => { load(); }, []);
  useEffect(() => { messagesEnd.current?.scrollIntoView({ behavior: 'smooth' }); }, [selected, threads]);

  const handleNewThread = async () => {
    if (!newThread.trim()) return;
    await fetch('/api/collaborator/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: newThread }) });
    setNewThread(''); load();
  };

  const handleSend = async () => {
    if (!selected || !newMessage.trim()) return;
    await fetch('/api/collaborator/chat/' + selected, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: newMessage }) });
    setNewMessage(''); load();
  };
          ...
          {!selected ? (
            ...
            <>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
                {messages.map((m, i) => (
                  <div key={i} style={{ maxWidth: '70%', alignSelf: m.sender === 'collaborator' ? 'flex-end' : 'flex-start' }}>
                    <div style={{ padding: '10px 14px', borderRadius: 12, background: m.sender === 'collaborator' ? 'var(--blue)' : 'var(--ink-100)', color: m.sender === 'collaborator' ? '#fff' : 'var(--ink-900)', fontSize: 14, lineHeight: 1.5 }}>{m.text}</div>
                    <div style={{ fontSize: 10, color: 'var(--ink-400)', marginTop: 4, textAlign: m.sender === 'collaborator' ? 'right' : 'left' }}>{new Date(m.timestamp).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</div>
                  </div>
                ))}
                <div ref={messagesEnd} />
              </div>
