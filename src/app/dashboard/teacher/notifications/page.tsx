...
export default function TeacherNotificationsPage() {
  ...
  useEffect(() => {
    fetch('/api/teacher/notifications').then(r => r.json()).then(d => { setNotifications(d); setLoading(false); });
  }, []);
    ...
    <div>
      ...
      {notifications.length === 0 ? (
        <div style={{ background: '#fff', borderRadius: 14, padding: 48, textAlign: 'center', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--text-mute)' }}>You&apos;ll be notified about submission approvals, rejections, and account changes.</p>
        </div>
      ) : (
