...
export const PATCH = withRateLimit(async function PATCH(
  ...
  // ponytail: admin_assistant can only update teacher/collaborator accounts
  if (role === "admin_assistant") {
    ...
    if (
      !target ||
      (target.role !== "teacher" && target.role !== "community_collaborator")
    ) {
      return NextResponse.json(
        {
          error: "Assistants can only update teacher or collaborator accounts",
        },
        { status: 403 }
      );
    }
  ...
  if (body.password) {
    ...
    await prisma.user.update({
      where: { id },
      data: { password: hashed, passwordIsTemporary: false },
    });
  ...
  const user = await prisma.user.update({
    ...
    data: {
      status: body.status,
      suspendedReason: body.suspendedReason || null,
    },
  });
...
export const DELETE = withRateLimit(async function DELETE(
  ...
  if (role === "admin_assistant") {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error: "Assistants can only delete teacher or collaborator accounts",
        },
        { status: 403 }
      );
    }
  ...
  await prisma.$transaction([
    ...
    prisma.notification.deleteMany({
      where: { OR: [{ recipientId: id }, { sentBy: id }] },
    }),
    prisma.communityCollaboratorSubmission.deleteMany({
      where: { submittedBy: id },
    }),
    prisma.chatThread.deleteMany({
      where: { OR: [{ openedBy: id }, { assignedTo: id }] },
    }),
    prisma.adminAssistantMessage.deleteMany({
