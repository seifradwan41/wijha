...
export default function SearchContent({
  ...
  const filteredSubjects = useMemo(() => {
    if (!category)
      return categories.flatMap((c) =>
        c.subcategories.map((s) => ({ id: s.id, name: s.name, parent: c.name }))
      );
  ...
  const activeFilters = [category, subject, level, examDate, query].filter(
    Boolean
  ).length;
      ...
      <section className="block">
        <div className="search-panel">
          ...
          <div className="filters">
            <div className={`filter${category ? " active" : ""}`}>
            ...
            <div className={`filter${subject ? " active" : ""}`}>
              <label>Subject</label>
              <select
                value={subject}
            ...
            <div className={`filter${level ? " active" : ""}`}>
              <label>Level</label>
              <select
                value={level}
            ...
            <div className={`filter${examDate ? " active" : ""}`}>
          ...
          >
            {activeFilters > 0
              ? `Filtered Courses (${filteredCourses.length} of ${courses.length})`
          ...
          <div className="results">
            {filteredCourses.length === 0 ? (
              ...
              filteredCourses.map((c) => (
                ...
                >
                  <span className={`tag ${c.category.toLowerCase()}`}>
                    {c.category}
                  </span>
                  <h4>{c.title}</h4>
                  <div className="meta">
                    {c.teacher.name} · {c.level} · {c.subcategory}
                  </div>
                    ...
                    {c.teacher.whatsappContact ? (
                      <span
                        ...
                        onClick={(e) => {
                          ...
                          e.stopPropagation();
                          window.open(
                            `https://wa.me/${c.teacher.whatsappContact!.replace(/[^0-9]/g, "")}?text=Hi!%20I'm%20interested%20in%20the%20course:%20${encodeURIComponent(c.title)}`,
                            "_blank"
                          );
                        }}
                      >
        ...
        <div style={{ marginTop: 64 }}>
          ...
          >
            <h2
              style={{ fontSize: 22, fontFamily: "Fraunces, serif", margin: 0 }}
            >
              {category || subject
                ? `Filtered Teachers (${filteredTeachers.length})`
          ...
          <div className="search-teacher-grid">
            {filteredTeachers.length === 0 ? (
              <p style={{ color: "var(--text-mute)", gridColumn: "1 / -1" }}>
              ...
              filteredTeachers.map((t) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h4>{t.name}</h4>
                    <span>
                      {t.categories.join(" · ")} · {t.specialties.join(", ")}
                    </span>
                    {t.whatsappContact && (
                      <span
                        onClick={(e) => {
                          ...
                          e.stopPropagation();
                          window.open(
                            `https://wa.me/${t.whatsappContact!.replace(/[^0-9]/g, "")}`,
                            "_blank"
                          );
                        }}
                        style={{
