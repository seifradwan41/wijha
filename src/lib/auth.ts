  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        const username = (credentials as Record<string, unknown>)
          .username as string;
  ...
  callbacks: {
    async jwt({ token, user, trigger, session }) {
      if (user) {
        ...
        token.onboardingCompletedAt =
          (
            user as unknown as { onboardingCompletedAt?: Date }
          )?.onboardingCompletedAt?.toISOString() || null;
        token.orientationSeenAt =
          (
            user as unknown as { orientationSeenAt?: Date }
          )?.orientationSeenAt?.toISOString() || null;
      }
      if (trigger === "update") {
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        (session.user as unknown as { _username?: string })._username =
          token.username as string;
        (session.user as unknown as { _suspended?: boolean })._suspended =
          token.status !== "active";
        (
          session.user as unknown as { _onboardingCompleted?: boolean }
        )._onboardingCompleted = !!token.onboardingCompletedAt;
        (
          session.user as unknown as { _orientationSeen?: boolean }
        )._orientationSeen = !!token.orientationSeenAt;
      }
      return session;
    },
  },
